...
import sys
sys.path.append('..')
from functions import get_instance, clean_gene_name
...
def add_panres_genes(file, onto, logger):
    ...
    for gene in genes:
        ...
        if m['gene_len'].nunique() == 1:
            new_gene.has_length_bp.append(int(m['gene_len'].values[0]))
        else:
        ...
        for _, row in m.iterrows():
            ...
            original_gene_instance.original_gene_is_from_database.append(database_instance)
            original_gene_instance.original_fasta_header.append(fasta_header + '|' + database_name)
